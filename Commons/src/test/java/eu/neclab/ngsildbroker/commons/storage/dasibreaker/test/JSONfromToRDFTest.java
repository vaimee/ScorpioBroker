...
public class JSONfromToRDFTest{
	...
	public void testJSONLDtoRDFtoJSONLD() throws JsonLdError{
		...
		try {
			System.out.println("rdf test result-->"+new JfromToRDF().JSONtoRDF(jsonld));
		} catch (Exception e) {
	...
	public void fullChain() throws JsonLdError{
		...
		try {
			String turtle = new JfromToRDF().JSONtoRDF(jsonld);
			...
			System.out.println("fullChain test result-->"+new JfromToRDF().RDFtoJson(qr.getBindingsResults().getBindings()));
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
